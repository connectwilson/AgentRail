...
// src/protocol.ts
var PROTOCOL_NAME = "AgentRail";
var PROTOCOL_VERSION = "0.2.0";
var PROTOCOL_SCHEMA_VERSION = "2026-04-21";
function withProtocolMeta(response) {
  return {
    ...response,
    meta: {
      ...response.meta,
...
var abiCache = new TtlCache(30 * 60 * 1000);
var abiNegativeCache = new TtlCache(5 * 60 * 1000);
function makeAbiCacheKey(chain, address, source) {
...
async function resolveAbi(params) {
  ...
  if ((params.fallbackStandards ?? []).length > 0) {
    const merged = params.fallbackStandards.flatMap((standard) => STANDARD_ABIS[standard] ?? []);
    if (merged.length > 0) {
      return { abi: merged, source: "built-in-standards" };
    }
  }
...
async function fetchSourcifyAbi(chain, address) {
  const cacheKey2 = makeAbiCacheKey(chain, address, "sourcify");
  const negKey = makeAbiCacheKey(chain, address, "sourcify:neg");
  const cached = abiCache.get(cacheKey2);
  if (cached)
    return JSON.parse(cached);
  if (abiNegativeCache.has(negKey))
    return null;
  ...
  for (const url of urls) {
    try {
      ...
      if (payload.output?.abi && payload.output.abi.length > 0) {
        abiCache.set(cacheKey2, JSON.stringify(payload.output.abi));
...
async function fetchExplorerAbi(chain, address) {
  const cacheKey2 = makeAbiCacheKey(chain, address, "explorer");
  ...
  try {
    ...
    const abi2 = normalizeAbiJson(payload.result);
    abiCache.set(cacheKey2, JSON.stringify(abi2));
    return abi2;
  } catch {
    abiNegativeCache.set(negKey, true);
    return null;
...
function normalizeAbiJson(raw) {
  const parsed = JSON.parse(raw);
  return Array.isArray(parsed) && typeof parsed[0] === "string" ? parseAbi(parsed) : parsed;
}
function requireAbi(result) {
  if (!result.abi) {
    throw new AcpError("ABI_REQUIRED", "No ABI available. Pass abi/abiPath, use a verified contract, or target a supported built-in standard.");
  }
  return result.abi;
}
function buildMinimalAbiFromSignature(params) {
  const stateMutability = params.stateMutability ?? "view";
  const returnsClause = params.returns && params.returns.length > 0 ? ` returns (${params.returns.join(",")})` : "";
  try {
    return parseAbi([
      `function ${params.functionSignature} ${stateMutability}${returnsClause}`.trim()
    ]);
  } catch (error) {
    throw new AcpError("INVALID_FUNCTION_SIGNATURE", "Unable to build ABI from function signature. Pass a full ABI or provide a valid signature like balanceOf(address).", { functionSignature: params.functionSignature, reason: error instanceof Error ? error.message : String(error) });
  }
}
...
async function loadAbiFromPath(abiPath) {
...
function listAbiFunctions(abi2) {
  return abi2.filter((item) => item.type === "function");
}
function listAbiEvents(abi2) {
  return abi2.filter((item) => item.type === "event");
}
function resolveFunction(abi2, functionId) {
  const functions = listAbiFunctions(abi2);
...
function formatFunctionSignature(item) {
  const inputs = item.inputs.map((input) => input.type).join(",");
  return `${item.name}(${inputs})`;
}
function semanticTypeFromInput(name, solidityType) {
  if (solidityType === "address") {
    return name?.includes("receiver") || name?.includes("owner") || name?.includes("spender") ? "wallet_address" : "address";
  }
...
function summarizeFunctionRisk(name) {
  const lowered = name.toLowerCase();
  if (lowered.includes("approve") || lowered.includes("setapprovalforall")) {
    return "high";
  }
  if (lowered.includes("transfer") || lowered.includes("deposit") || lowered.includes("withdraw") || lowered.includes("mint") || lowered.includes("burn")) {
    return "medium";
  }
  return "low";
}
...
function decodeLogWithAbi(params) {
  try {
...
function assertWriteAllowed(params) {
  ...
  if (!policy.allowWrites) {
    throw new AcpError("WRITE_BLOCKED", "Writes are blocked by policy. Set policy.allowWrites=true to continue.");
  }
  if (policy.approvedContracts && !policy.approvedContracts.includes(params.contractAddress)) {
    throw new AcpError("CONTRACT_NOT_APPROVED", "Target contract is not in policy.approvedContracts.", { address: params.contractAddress });
  }
  ...
  if (policy.maxValueWei && params.value) {
    ...
    if (requested > max) {
      throw new AcpError("VALUE_LIMIT_EXCEEDED", `Call value ${requested} exceeds policy max ${max}.`);
    }
  }
}
...
_loadExternalFile();
function lookupRegistry(params) {
...
function getAaveMarketEntries(chain) {
  return REGISTRY.filter((entry) => entry.chain === chain && entry.protocol === "aave" && entry.category === "market");
}
...
function getWalletClient(chain, privateKey) {
  const key = privateKey ?? getEnv("ACP_PRIVATE_KEY") ?? getEnv("PRIVATE_KEY");
  if (!key) {
...
async function getBytecode(chain, address) {
...
class NonceManager {
  ...
  async acquire(chain, address, fetchNonce) {
...
var SUPPORTED_CHAIN_IDS = new Set(Object.keys(CHAIN_ID_TO_NAME).map(Number));
...
async function findTokensBySymbol(symbol, chain) {
  const tokens = await getTokenList();
  const needle = symbol.toLowerCase();
  return tokens.filter((t) => {
    if (t.symbol.toLowerCase() !== needle)
      return false;
    if (chain) {
...
async function findTokensByName(name, chain) {
  const tokens = await getTokenList();
...
function tokenListEntryToHint(entry) {
  return {
...
var _cachedProtocols = null;
var _cacheExpiresAt2 = 0;
var CACHE_TTL_MS2 = 60 * 60 * 1000;
...
async function getProtocols() {
  if (_cachedProtocols && Date.now() < _cacheExpiresAt2) {
  ...
  if (protocols.length > 0) {
    ...
    _cacheExpiresAt2 = Date.now() + CACHE_TTL_MS2;
    logger.info("defillama.loaded", { count: protocols.length });
  }
  return protocols;
}
async function findProtocols(query, chain) {
  const protocols = await getProtocols();
  const needle = query.toLowerCase();
  const matches = protocols.filter((p) => {
    const nameMatch = p.name.toLowerCase().includes(needle) || p.slug.toLowerCase().includes(needle) || (p.symbol ?? "").toLowerCase() === needle;
    if (!nameMatch)
      return false;
    if (chain) {
      const llamaChains = (p.chains ?? []).map((c) => c.toLowerCase());
      const targetLlamaNames = Object.entries(LLAMA_CHAIN_MAP).filter(([, v]) => v === chain).map(([k]) => k);
...
function llamaProtocolToHint(p, chain) {
  const supportedChains = (p.chains ?? []).map((c) => LLAMA_CHAIN_MAP[c.toLowerCase()]).filter(Boolean);
  return {
    name: p.name,
    slug: p.slug,
...
async function detectStandards(params) {
  ...
  const functionSurface = async (abi2, functions) => {
    const available = await Promise.all(functions.map(async (fnName) => {
      try {
        const fn = resolveFunction(abi2, fnName);
...
async function getStandardsIfNeeded(params) {
  if (params.abi || params.abiPath || "returns" in params && Array.isArray(params.returns) || "stateMutability" in params && typeof params.stateMutability === "string") {
    return [];
  }
  return detectStandards(params);
}
async function getImplementationAddress(chain, address) {
...
async function getPreferredAbi(params, standards, defaultStateMutability = "view") {
  return resolveAbi({
    chain: params.chain,
    ...
    functionSignature: "function" in params && typeof params.function === "string" ? params.function : undefined,
    returns: "returns" in params && Array.isArray(params.returns) ? params.returns : undefined,
    stateMutability: "stateMutability" in params && typeof params.stateMutability === "string" ? params.stateMutability : defaultStateMutability,
    loadLocalAbi: loadAbiFromPath
  });
}
function buildEventOnlyAbi(abis) {
  return abis.flatMap((abi2) => listAbiEvents(abi2));
}
function summarizeDecodedEvent(eventName, args) {
...
function normalizeDecodedArgs(args) {
  if (!args || Array.isArray(args) || typeof args !== "object") {
    return {};
  }
  return args;
}
function hasBalanceFields(position) {
  return "formatted" in position || "decoded" in position;
}
async function contractInspect(params) {
  const address = normalizeAddress(params.address);
  const bytecode = await getBytecode(params.chain, address);
  const standards = bytecode ? await getStandardsIfNeeded(params) : [];
  const abiResult = bytecode ? await getPreferredAbi(params, standards) : { abi: null, source: "none" };
  ...
  return {
    ...
    result: {
      ...
      riskHints: implementation ? ["Upgradeable proxy detected; behavior may change if implementation is upgraded."] : []
    },
    meta: {
      chain: params.chain,
      chainId: getChainConfig(params.chain).chainId,
...
async function contractFunctions(params) {
  ...
  const functions = listAbiFunctions(abi2).map((item) => ({
    name: item.name,
...
async function contractDescribe(params) {
  const standards = await getStandardsIfNeeded(params);
  const abiResult = await getPreferredAbi(params, standards, "view");
  const abi2 = requireAbi(abiResult);
...
function summarizeFunction(name) {
  const lowered = name.toLowerCase();
...
async function getTokenMetadata(params) {
  ...
  if (symbol !== null && decimals !== null) {
    return { symbol, decimals, source: "request" };
  }
  try {
    if (symbol === null) {
  ...
  try {
    if (decimals === null) {
      ...
      decimals = typeof decimalsRead.raw === "bigint" ? Number(decimalsRead.raw) : Number(formatDisplayValue(decimalsRead.raw));
...
async function contractRead(params) {
...
async function registryLookup(params) {
  const entries = lookupRegistry(params);
  const bestMatch = entries[0] ?? null;
  let onChainHint = null;
  const addressLike = (params.address ?? params.query)?.match(/^0x[0-9a-fA-F]{40}$/i);
  const addressNotInRegistry = !!addressLike && (entries.length === 0 || !entries.some((e) => e.address?.toLowerCase() === addressLike[0].toLowerCase()));
  if (addressNotInRegistry && params.chain) {
    ...
    try {
      ...
      const [symbol, name, decimals] = await Promise.all([
        client.readContract({ address: candidate, abi: probeAbi, functionName: "symbol", args: [] }).catch(() => null),
        client.readContract({ address: candidate, abi: probeAbi, functionName: "name", args: [] }).catch(() => null),
        client.readContract({ address: candidate, abi: probeAbi, functionName: "decimals", args: [] }).catch(() => null)
      ]);
      if (symbol || name) {
        onChainHint = {
          address: candidate,
  ...
  if (entries.length === 0 && !onChainHint) {
    ...
    if (sym) {
      ...
      const byName = bySymbol.length === 0 ? await findTokensByName(qry, params.chain).catch(() => []) : [];
  ...
  const summary = bestMatch ? `Found ${entries.length} registry match${entries.length === 1 ? "" : "es"}; best match is ${bestMatch.name}${bestMatch.address ? ` at ${bestMatch.address}` : ""}.` : onChainHint ? `Not in registry, but on-chain ERC20 probe found: ${String(onChainHint.symbol ?? onChainHint.name)}. Call registry.add to register.` : tokenListHints.length > 0 ? `Not in registry, but found ${tokenListHints.length} match(es) in the Uniswap token list. See tokenListHints.` : llamaHints.length > 0 ? `Not in registry, but found ${llamaHints.length} match(es) on DeFiLlama. See llamaHints for details.` : "No registry matches found. See suggestions for next steps.";
  return {
    id: crypto.randomUUID(),
    ok: true,
    result: {
...
async function tokenBalance(params) {
  ...
  return {
    ...
    result: {
      ...
      formatted: read.result?.formatted ?? formatTokenValue(read.result?.decoded ?? null, metadata.decimals ?? undefined),
...
async function aavePositions(params) {
  const assets = params.assets ?? getAaveMarketEntries(params.chain).map((entry) => ({
    symbol: entry.symbol ?? entry.name,
    aTokenAddress: entry.address ?? "",
    decimals: typeof entry.metadata?.decimals === "number" ? entry.metadata.decimals : undefined
  }));
  const positions = await Promise.all(assets.map(async (asset) => {
    try {
      ...
      const isSupplied = typeof formatted === "string" ? Number(formatted) > 0 : typeof balance.result?.decoded === "string" ? BigInt(balance.result.decoded) > 0n : false;
      return {
        symbol: asset.symbol,
      ...
      const normalized = error instanceof AcpError ? error : new AcpError("AAVE_POSITION_READ_FAILED", error instanceof Error ? error.message : String(error));
      return {
        symbol: asset.symbol,
        aTokenAddress: asset.aTokenAddress,
        supplied: false,
  ...
  const highlights = nonZeroPositions.map((position) => {
    const value = hasBalanceFields(position) ? typeof position.formatted === "string" ? position.formatted : typeof position.decoded === "string" ? position.decoded : null : null;
    return {
  ...
  const summary = nonZeroPositions.length === 0 ? `No supplied Aave positions were found across ${positions.length} tracked assets on ${params.chain}.` : `Found ${nonZeroPositions.length} non-zero Aave supplied position${nonZeroPositions.length === 1 ? "" : "s"} across ${positions.length} tracked assets on ${params.chain}: ${nonZeroPositions.map((position) => {
    const value = hasBalanceFields(position) ? typeof position.formatted === "string" ? position.formatted : typeof position.decoded === "string" ? position.decoded : null : null;
...
async function contractSimulate(params) {
  const address = normalizeAddress(params.address);
  const standards = await getStandardsIfNeeded(params);
...
function buildHumanSummary(name, args) {
...
async function txBuild(params) {
  ...
  const gas = params.gas ? BigInt(params.gas) : await client.estimateContractGas({
    account: caller,
    address,
    abi: abi2,
    functionName: fn.name,
  ...
  const nonce = params.nonce !== undefined ? params.nonce : await client.getTransactionCount({ address: caller });
  return {
    id: crypto.randomUUID(),
    ok: true,
    result: {
      ...
      maxPriorityFeePerGas: params.maxPriorityFeePerGas ?? fees.maxPriorityFeePerGas?.toString() ?? null,
      warnings: [
...
async function txSend(params) {
  ...
  if (caller !== signerAddress) {
    throw new AcpError("SIGNER_CALLER_MISMATCH", "caller must match the configured signer address for tx.send.", { caller, signerAddress });
  }
  ...
  const gas = params.gas ? BigInt(params.gas) : await publicClient.estimateContractGas({
    account: signerAddress,
    address,
    abi: abi2,
    functionName: fn.name,
  ...
  const nonce = params.nonce !== undefined ? params.nonce : await nonceManager4.acquire(params.chain, signerAddress, () => publicClient.getTransactionCount({ address: signerAddress }));
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      gas,
      nonce,
      maxFeePerGas: params.maxFeePerGas ? BigInt(params.maxFeePerGas) : fees.maxFeePerGas,
      maxPriorityFeePerGas: params.maxPriorityFeePerGas ? BigInt(params.maxPriorityFeePerGas) : fees.maxPriorityFeePerGas
    });
    logger.info("tx.send.broadcast", { chain: params.chain, hash: hash3, nonce });
    return {
      id: crypto.randomUUID(),
      ok: true,
      result: {
    ...
    nonceManager4.reset(params.chain, signerAddress);
...
async function receiptDecode(params) {
  const client = getPublicClient(params.chain);
  ...
  const effects = decodedLogs.filter((log) => log.decoded && ("summary" in log)).map((log) => ({
    address: log.address,
    eventName: log.eventName,
    effect: log.summary
  }));
  const summary = effects.length === 0 ? `Decoded receipt for ${params.hash} with no recognized token or approval events.` : `Decoded receipt for ${params.hash}; recognized ${effects.length} effect${effects.length === 1 ? "" : "s"}.`;
  return {
    id: crypto.randomUUID(),
    ok: true,
    result: {
...
async function actionPlan(params) {
...
async function compoundPositions(params) {
  const registryMarkets = getCompoundMarkets(params.chain);
  const markets = params.markets ?? registryMarkets.map((entry) => ({
    address: entry.address ?? "",
  ...
  const positions = await Promise.all(markets.filter((m) => Boolean(m.address)).map(async (market) => {
    try {
  ...
  const activePositions = positions.filter((p) => !("error" in p) && (p.hasSupply || p.hasBorrow));
  const summary = activePositions.length === 0 ? `No active Compound V3 positions found for ${params.owner} across ${positions.length} markets on ${params.chain}.` : `Found ${activePositions.length} active Compound V3 position${activePositions.length === 1 ? "" : "s"} on ${params.chain}: ${activePositions.map((p) => `${p.baseToken}(supply=${p.supplyFormatted ?? p.supplyRaw},borrow=${p.borrowFormatted ?? p.borrowRaw})`).join(", ")}.`;
  return {
    id: crypto.randomUUID(),
    ok: true,
    result: {
...
async function uniswapQuote(params) {
  const quoterAddress = params.quoterAddress ?? DEFAULT_QUOTER_ADDRESSES[params.chain];
  if (!quoterAddress) {
    throw new AcpError("QUOTER_NOT_FOUND", `No Uniswap V3 QuoterV2 address known for chain '${params.chain}'. Provide quoterAddress explicitly.`);
  }
  ...
  const [tokenInDecimals, tokenOutDecimals, tokenInSymbol, tokenOutSymbol] = await Promise.all([
    client.readContract({ address: tokenIn, abi: STANDARD_ABIS.ERC20, functionName: "decimals", args: [] }).catch(() => {
      return;
    }),
    client.readContract({ address: tokenOut, abi: STANDARD_ABIS.ERC20, functionName: "decimals", args: [] }).catch(() => {
      return;
    }),
    client.readContract({ address: tokenIn, abi: STANDARD_ABIS.ERC20, functionName: "symbol", args: [] }).catch(() => {
      return;
    }),
    client.readContract({ address: tokenOut, abi: STANDARD_ABIS.ERC20, functionName: "symbol", args: [] }).catch(() => {
      return;
    })
  ]);
  try {
    ...
    const amountInFormatted = formatTokenValue(amountIn, tokenInDecimals);
    const amountOutFormatted = formatTokenValue(amountOut, tokenOutDecimals);
    const summary = `${amountInFormatted ?? amountIn.toString()} ${tokenInSymbol ?? tokenIn} → ${amountOutFormatted ?? amountOut.toString()} ${tokenOutSymbol ?? tokenOut} (fee=${fee / 1e4}%)`;
    return {
      id: crypto.randomUUID(),
      ok: true,
      result: {
...
async function executeRequest(request) {
  ...
  if (request.method === "rpc.schema") {
    const method = request.params && typeof request.params.method === "string" ? request.params.method : undefined;
    if (!method) {
