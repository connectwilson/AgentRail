...
async function registryLookup(params) {
  ...
  const summary = bestMatch ? `Found ${entries.length} registry match${entries.length === 1 ? "" : "es"}; best match is ${bestMatch.name}${bestMatch.address ? ` at ${bestMatch.address}` : ""}.` : "No registry matches found for the query.";
  return {
    id: crypto.randomUUID(),
    ok: true,
    result: {
...
async function txSend(params) {
  ...
  const nonce = params.nonce !== undefined ? params.nonce : await publicClient.getTransactionCount({ address: signerAddress });
  try {
